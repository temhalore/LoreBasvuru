...
export class LayoutComponent implements OnInit, OnDestroy {
     ...
     */
    ngOnInit(): void {
        // Kullanıcının daha önce seçtiği layout/scheme/theme varsa uygula (config varsayılanını geçer)
        const savedLayout = LocalStorageService.getSelectedLayout();
        const savedScheme = LocalStorageService.getSelectedScheme();
        const savedTheme = LocalStorageService.getSelectedTheme();
        const savedPrefs: Partial<FuseConfig> = {};
        if (savedLayout) { savedPrefs.layout = savedLayout; }
        if (savedScheme) { savedPrefs.scheme = savedScheme as any; }
        if (savedTheme) { savedPrefs.theme = savedTheme; }
        if (Object.keys(savedPrefs).length > 0) {
            this._fuseConfigService.config = savedPrefs;
        }
